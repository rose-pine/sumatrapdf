<p align="center">
    <img src="https://github.com/rose-pine/rose-pine-theme/raw/main/assets/icon.png" width="80" />
    <h2 align="center">Rosé Pine for SumatraPDF</h2>
</p>

<p align="center">All natural pine, faux fur and a bit of soho vibes for the classy minimalist</p>

<p align="center">
    <a href="https://github.com/rose-pine/rose-pine-theme">
        <img src="https://img.shields.io/badge/community-rosé%20pine-26233a?labelColor=191724&logo=data:image/svg+xml;base64,&style=for-the-badge" />
    </a>
</p>

## Usage

1. Download any of the `.txt` files  that contains a theme variant.
2. Open SumatraPDF > Settings > Advanced Options...
3. Copy contents of the downloaded file into the settings file.

## Gallery

**Rosé Pine**

![Rosé Pine with SumatraPDF](https://github.com/user-attachments/assets/6df08169-54cc-47aa-b6bc-9f2304c3ccaa)

**Rosé Pine Moon**

![Rosé Pine Moon with SumatraPDF](https://github.com/user-attachments/assets/8b29281c-64d1-4ad1-a8a8-2771e7dba198)

**Rosé Pine Dawn**

![Rosé Pine Dawn with SumatraPDF](https://github.com/user-attachments/assets/e65521de-6042-426e-8bd9-c4fd5ef68cc0)

## Thanks to

- [darkthemer](https://github.com/darkthemer)
